...
public class ModificaOrdineSelezionato extends javax.swing.JFrame {
    ...
    public ArrayList<Catalogo_ModificaOrdine> modificaList() throws ClassNotFoundException, SQLException{
        ...
        String query = "SELECT o.idOrdine, fo.idFarmaco, f.nome, f.principio, f.scadenza, o.dataConsegna, f.quantita, fo.quantita FROM farmaco f INNER JOIN farmacoordine fo ON f.idFarmaco = fo.idFarmaco INNER JOIN ordine o ON o.idOrdine = fo.idOrdine INNER JOIN utente u ON u.id = o.idUtente WHERE u.id = '"+id+"' AND o.stato = \"In preparazione\" AND o.idOrdine = '"+idOrdine+"'";
        Statement st = connessione.createStatement();
        ResultSet rs = st.executeQuery(query);
        Catalogo_ModificaOrdine check;
        while(rs.next()){
            check = new Catalogo_ModificaOrdine(rs.getString("o.idOrdine"), rs.getString("fo.idFarmaco"), rs.getString("f.nome"), rs.getString("f.principio"), rs.getString("f.scadenza"), rs.getString("o.dataConsegna") ,rs.getString("f.quantita"), rs.getString("fo.quantita"));
    ...
    public void mostraModifiche() throws ClassNotFoundException, SQLException{
        ...
        for(int i=0;i<check.size(); i++){
            ...
            righe[6] = check.get(i).getqDisponibili();
            righe[7] = check.get(i).getqAttuale();
            model.addRow(righe);
        }
    }
    ...
    private void initComponents() {
        ...
        tabellaModifica.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "ID Ordine", "ID Farmaco", "Nome Farmaco", "Principio Attivo", "Data scadenza", "Data Consegna", "Disponibili", "Q.ty Attuale", "Q.ty Nuova"
            }
        ) {
        ...
        try{
            ...
            for(int i=0; i<tabellaModifica.getRowCount();i++){
                if(tabellaModifica.isRowSelected(i)){
                    ...
                    disponibili = (tabellaModifica.getModel().getValueAt(i, 6).toString());
